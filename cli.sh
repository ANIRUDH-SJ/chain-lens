...
if [[ "${1:-}" == "--block" ]]; then
  ...
  # Run the block parser via Node.js/TypeScript
  ...
  exec npx tsx "$SCRIPT_DIR/src/cli.ts" --block "$BLK_FILE" "$REV_FILE" "$XOR_FILE"
fi
...
# Run the transaction analyzer via Node.js/TypeScript
...
exec npx tsx "$SCRIPT_DIR/src/cli.ts" "$FIXTURE"
